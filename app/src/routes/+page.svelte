<script lang="ts">
	...
	const timeError = $derived(
		draft.hasTimeLimit && (!Number.isFinite(draft.timeLimitMinutes) || draft.timeLimitMinutes < 1 || draft.timeLimitMinutes > 180)
			? 'Choose a time limit from 1 to 180 minutes.'
			: ''
	);
	...
	const timeSummary = $derived(
		draft.hasTimeLimit
			? `${draft.timeLimitMinutes} min ${draft.timeMode === 'per-question' ? 'per question' : 'total'}`
			: 'No time limit'
	);
	...
	onMount(() => {
		...
		if (saved) {
			try {
				const parsed = JSON.parse(saved) as Partial<QuizDraft>;
				...
				draft = {
					...
					difficulty,
					timeMode,
					timeLimitMinutes: Number(parsed.timeLimitMinutes) || defaultDraft.timeLimitMinutes
				};
				saveState = 'loaded';
			} catch {
						...
						<div class="time-controls">
							<label class="field compact">
								...
								<input
									...
									max="180"
									step="1"
									bind:value={draft.timeLimitMinutes}
